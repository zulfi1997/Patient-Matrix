...
function KpiRow({ kpi, records, patients, asOfISO }: { kpi: StaffScorecardKpi; records: SaleRecord[]; patients: Map<string, PatientVisitSummary>; asOfISO: string }) {
  ...
  const computation = definition ? definition.compute(records, patients, target, asOfISO) : null;
...
function ScorecardCard({
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, StaffScorecardKpi[]>();
    for (const kpi of scorecard.kpis) {
      const list = map.get(kpi.category);
      if (list) list.push(kpi);
      else map.set(kpi.category, [kpi]);
    }
    return [...map.entries()];
  }, [scorecard.kpis]);

  const computableCount = scorecard.kpis.filter((k) => findKpiDefinition(k.metric)).length;
        ...
        <div key={category} className="mb-4 last:mb-0">
          ...
          <div className="overflow-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {kpis.map((kpi, i) => (
                  <KpiRow key={i} kpi={kpi} records={records} patients={patients} asOfISO={asOfISO} />
                ))}
              </tbody>
            </table>
          </div>
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Upload a staff member's offer letter (.docx) with a Key Performance Indicators section - KPIs that can be
          measured from clinic sales data (revenue, new patients, package sales, retention) are tracked
          automatically against overall clinic performance; anything that needs a CRM, survey, or partnership log
          (leads, NPS, partnerships, compliance, etc.) is shown as needing manual tracking instead of guessed at.
        </p>
      </div>
      ...
      {scorecards.length === 0 ? (
        ...
        scorecards.map((s) => (
          <ScorecardCard key={s.id} scorecard={s} records={records} patients={patients} asOfISO={asOfISO} onRemove={removeScorecard} />
        ))
      )}
    </div>
  );
