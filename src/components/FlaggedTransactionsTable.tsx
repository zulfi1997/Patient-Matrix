...
function downloadCsv(rows: FlaggedTransaction[]) {
  const header = ['Patient ID', 'Patient Name', 'Date', 'Service', 'Item Type', 'Staff', 'Invoice No', 'Amount (OMR)', 'Invoice Notes'];
  const lines = rows.map((r) =>
    [r.patientId, r.patientName, r.date, r.serviceName, r.itemType, r.staff ?? '', r.invoiceNo, r.amount.toFixed(3), r.notes]
