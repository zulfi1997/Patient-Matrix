...
import type { FlaggedDueInvoice } from '../lib/metrics';
import { formatCurrency, formatDate, formatNumber } from '../lib/format';

const PAGE_SIZE = 25;

function downloadCsv(rows: FlaggedDueInvoice[]) {
  const header = ['Invoice No', 'Patient ID', 'Patient Name', 'Date', 'Services', 'Due Amount (OMR)', 'Invoice Status'];
  const lines = rows.map((r) =>
    [r.invoiceNo, r.patientId, r.patientName, r.date, r.services.join('; '), r.dueAmount.toFixed(3), r.invoiceStatus]
  ...
  a.href = url;
  a.download = `yb111-due-invoices-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
  URL.revokeObjectURL(url);
}

export function FlaggedDueInvoicesTable({ data, periodLabel }: { data: FlaggedDueInvoice[]; periodLabel: string }) {
  ...
  const totalDue = useMemo(() => filtered.reduce((sum, r) => sum + r.dueAmount, 0), [filtered]);
    ...
    <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm print:break-inside-avoid print:bg-white dark:border-zinc-800 dark:bg-zinc-900">
      <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-200">
          "YB111" Invoices Due to Be Settled ({formatNumber(data.length)})
        </h3>
        <button
      ...
      <p className="mb-2 text-xs text-zinc-500 dark:text-zinc-400">
        {periodLabel}. Invoices with a "YB111"-flagged line and an outstanding balance, sorted by amount due.
        Total shown: {formatCurrency(totalDue)}.
      </p>
      <input
        type="text"
      ...
      {filtered.length === 0 ? (
        <p className="py-8 text-center text-sm text-zinc-500">No "YB111"-flagged invoices with an outstanding balance.</p>
      ) : (
        <>
